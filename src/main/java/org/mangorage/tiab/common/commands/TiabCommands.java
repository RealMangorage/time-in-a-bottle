                ...
                for (int i = 0; i < player.getInventory().getContainerSize(); i++) {
                    ...
                    if (item instanceof TiabItem itemTiab) {
                        ...
                        if (CommonHelper.isPositive(currentStoredEnergy + timeToAddFinal * CommonRegistration.SERVER_CONFIG.get().TICKS_CONST())) {
                        	...
                        	int stored = invStack.getOrCreateTag().getInt(NBTKeys.STORED_TIME);
                        	int accumulated = invStack.getOrCreateTag().getInt(NBTKeys.TOTAL_ACCUMULATED_TIME);
                        	
                        	CommonHelper.modify(invStack, () -> new StoredTimeComponent(stored, accumulated), old -> {
